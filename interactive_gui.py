...
class App(QWidget):
    def __init__(self, prop_net, fuse_net, s2m_ctrl:S2MController, fbrs_ctrl:FBRSController, 
                    images, masks, num_objects, mem_freq, mem_profile, step, num_ref, neighbor_stride, savefps,args):
        super().__init__()
        self.args = args
        self.images = images
        self.masks = masks
        ...
        self.ref_length = step  # ref_step
        self.num_ref = num_ref
        self.neighbor_stride = neighbor_stride
        self.default_fps = savefps
        ...
        self.run_button = QPushButton('Make it Vanish!')
    ...
    def read_mask(self, mpath):
        masks = []
        mnames = os.listdir(mpath)
        mnames.sort()
        for m in mnames: 
            m = Image.open(os.path.join(mpath, m))
            m = m.resize((self.width, self.height), Image.NEAREST)
    ...
    def read_frame_from_videos(self, path):
        vname = path
        frames = []
        if args.use_mp4:
            ...
            for fr in fr_lst:
                image = cv2.imread(fr)
                image = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
                frames.append(image.resize((self.width, self.height)))
        return frames  
    
    '''
    def dstt_inpaint(self, fpath, mpath):
        # set up models 
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        net = importlib.import_module('DSTT.model.' + args.model)
        ...
        frames = self.read_frame_from_videos(fpath)
        video_length = len(frames)
        imgs = _to_tensors(frames).unsqueeze(0)*2-1
        frames = [np.array(f).astype(np.uint8) for f in frames]

        masks = self.read_mask(mpath)
        ...
        writer = cv2.VideoWriter(f"{name}_result.mp4", cv2.VideoWriter_fourcc(*"mp4v"), self.default_fps, (args.outw, args.outh))
        for f in range(video_length):
            comp = np.array(comp_frames[f]).astype(
                np.uint8)*binary_masks[f] + frames[f] * (1-binary_masks[f])
            if self.width != args.outw:
    ...
    def dstt_inpaint(self):
        # set up models 
        self.console_push_text('Start Inpainting...')
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        net = importlib.import_module('dstt.model.' + args.model)
        model = net.InpaintGenerator().to(device)
        model_path = args.ckpt
        ...
        for f in range(0, video_length, self.neighbor_stride):
            ...
            ref_ids = self.get_ref_index(f, neighbor_ids, video_length)
            print(f, len(neighbor_ids), len(ref_ids))
            self.console_push_text('Progress:{}'.format(f))
            len_temp = len(neighbor_ids) + len(ref_ids)
            selected_imgs = imgs[:1, neighbor_ids+ref_ids, :, :, :]
            selected_masks = masks[:1, neighbor_ids+ref_ids, :, :, :]
            with torch.no_grad():
    ...
    def save(self):
        ...
        img_dir = path.join(folder_path, 'images')
        ...
        for i in range(self.num_frames):
            ...
            # Save image
            images = Image.fromarray(self.images[i]) 
            images.save(os.path.join(img_dir, '{:05d}.png'.format(i)))
        self.console_push_text('Done.')

        # self.dstt_inpaint(img_dir, mask_dir)
...
if __name__ == '__main__':
    ...
    with torch.cuda.amp.autocast(enabled=not args.no_amp):
        ...
        ex = App(prop_model, fusion_model, s2m_controller, fbrs_controller, 
                images, masks, num_objects, args.mem_freq, args.mem_profile, args.step, args.num_ref, args.neighbor_stride, args.savefps,args)
        sys.exit(app.exec_())
